...
namespace PPD
{
    public class NextSceneLoadHandler : SingletonMonoBehaviour<NextSceneLoadHandler>
    {
        ...
        void Begin(bool shake, TweenCallback callback)
        {
            ...
            blackoutImage.color = Data.Templates.nextSceneLoadHandler_defaultColor;
            canvas.transform.SetParent(null, false);
            DontDestroyOnLoad(canvas.gameObject);
            canvas.SetActiveSelf(true);
            ...
            if (shake)
            {
                ...
                .AppendInterval(Data.Templates.nextSceneLoadHandler_shakeSec);
            }

            seq.Append(blackoutImage.DOColor(Data.Templates.nextSceneLoadHandler_peakColor, Data.Templates.nextSceneLoadHandler_fadeoutSec))
            .AppendCallback(() => Time.timeScale = 1)
            .AppendCallback(callback)
            .Append(blackoutImage.DOColor(Data.Templates.nextSceneLoadHandler_defaultColor, Data.Templates.nextSceneLoadHandler_fadeinSec))
            .OnKill(() =>
            {
                if (canvas)
                {
